...
class MaskOnlineLayer(Layer):
    ...
    def __init__(self, dict_layer, templates: dict, debug=False):
        ...
        if self.debug:
            ...
            for key, entry in tqdm(templates.items(), disable=True):
                self.trie.insert(entry, tot)
                tot += 1
    ...
    def run(self, line, log_entry, encode_to_indices):
        ...
        if len(wordset) == 0:
            self.cluster2Template[self.lineID] = self.zeroTemplate
            self.template2Cluster[self.zeroTemplate] = self.lineID
            ret_val = self.zeroTemplate
            if encode_to_indices:
                ret_val = self.zeroTemplateIndex
            return ret_val

        if wordset in self.templates:
            existTemplate = self.orderedTemplates.get(tuple(line['dwords']))
            if existTemplate is not None:
                ret_val = ''.join(self.cluster2Template[existTemplate])
                if encode_to_indices:
                    ret_val = existTemplate
                return ret_val
            ...
            if updated_template == old_template:
                ...
                ret_val = ''.join(updated_template)
                if encode_to_indices:
                    ret_val = self.tagMap[tag]

            else:
                tag = self.trie.update(old_template, updated_template,self.cluster2Template, self.tagMap)
            ...
            if tag == -1:
                ...
                ret_val = ''.join(content)
                if encode_to_indices:
                    ret_val = tag
            else:
                self.templates[wordset] = self.tagMap[tag]
                ret_val = ''.join(self.cluster2Template[self.tagMap[tag]])
                if encode_to_indices:
                    ret_val = self.tagMap[tag]
